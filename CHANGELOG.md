...
- **`group_whitelist` 留空语义改为全部不生效**：
  之前留空表示所有群生效；现在 `group_whitelist` 为空列表时，本插件的
  状态观察、LLM 注入、智能引用、命令与群成员工具都不会生效，只有填入
  纯数字群号后才对指定群生效。
  - 白名单匹配使用 `event.get_group_id()`，即纯数字群号（如 `708947555`），
    不要填 `shelly:GroupMessage:708947555` 这类带标识的字符串。
...
- **修复：高级模板旧配置覆盖 v0.8.5 新判断逻辑**——v0.8.5 已在代码默认 prompt 中加入「最近对话原文」和第二人称判定规则，但旧配置文件里如果保存了 `judge_decision_prompt` / `judge_prompt_template` 的旧内置默认值，运行时会优先使用配置里的非空模板，导致新规则被旧模板覆盖。
  - `_config_template_or_default` 新增旧默认识别：当配置值等于旧版内置默认模板时，自动使用当前默认模板；真正手动改过的模板保持原样，不强行覆盖。
  - 覆盖两类旧默认：v0.8.4 schema 版模板，以及更早期 minimal 模板（你的本地配置文件属于这一类）。
- **预填充：同步 `_conf_schema.json` 高级模板默认值**——`judge_decision_prompt` 现在默认包含输入安全说明 + 主语判定；`judge_prompt_template` 默认包含当前消息对象、最近对话原文、历史分层、主语优先级和第二人称判定。
- 本机实际配置 `data/config/astrbot_plugin_social_context_config.json` 已同步预填新模板（不纳入仓库提交）。
- 测试：143 → 145 个用例（新增旧默认模板自动升级回归）。145 passed / 0 failed。
- ruff 0 issue。metadata.yaml v0.8.5 → v0.8.6。
