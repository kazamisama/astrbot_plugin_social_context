...
| tier2 | 180s ~ 30min | 压缩摘要（待接 LLM） | 占位（v0.6.0）|
| tier3 | 30min ~ 24h | 更早期压缩摘要（待接 LLM） | 占位（v0.6.0）|

**v0.6.0 本版只落地"时间结构"层**（数据模型 / 配置 / 持久化 / 过期抛弃 / 注入占位），LLM 压缩内容留待下个 patch。届时 judge prompt 会自动把摘要从"暂无"换成实际压缩文本。
